<template>
  <main class="shop-home">
    ...
    <section id="discover" class="discovery-section">
      ...
      <div class="idea-grid">
        <button v-for="item in moodIdeas" :key="item.value" type="button" :class="['idea-card',{active:mood===item.value}]" @click="chooseMood(item.value)">
    ...
    <section class="guided-search">
      <div>
        ...
        <h2>Who is it for?</h2>
        <div class="chips"><button v-for="item in recipientOptions" :key="item" :class="{active:recipient===item}" @click="recipient=recipient===item?'':item">{{ item }}</button></div>
      </div>
      <div>
        <p class="eyebrow">What is the moment?</p>
        <h2>Choose an occasion</h2>
        <div class="chips"><button v-for="item in occasionOptions" :key="item" :class="{active:occasion===item}" @click="occasion=occasion===item?'':item">{{ item }}</button></div>
      </div>
    </section>
...
<style scoped>
.shop-home{max-width:1240px;margin:auto;padding:1.5rem 1rem 4rem;color:var(--bb-text)}.hero{display:grid;grid-template-columns:1.55fr .75fr;gap:2rem;padding:clamp(2rem,5vw,4.5rem);border:1px solid var(--bb-border);border-radius:28px;background:linear-gradient(135deg,var(--bb-surface),color-mix(in srgb,var(--bb-primary-light) 12%,var(--bb-surface)))}h1{font-size:clamp(2.5rem,6vw,5.2rem);line-height:.98;letter-spacing:-.055em;max-width:820px;margin:.45rem 0 1rem}.hero-text{font-size:1.15rem;line-height:1.65;max-width:650px;color:var(--bb-muted)}.eyebrow{font-size:.76rem;font-weight:900;letter-spacing:.11em;text-transform:uppercase;color:var(--bb-primary-dark)}.hero-actions{display:flex;gap:.7rem;flex-wrap:wrap;margin-top:1.5rem}.btn{border:1px solid var(--bb-border);border-radius:999px;padding:.75rem 1rem;font-weight:800;cursor:pointer;text-decoration:none}.primary{background:var(--bb-primary-dark);color:white}.secondary{background:var(--bb-surface);color:var(--bb-text)}.promise{align-self:center;padding:1.4rem;border:1px solid var(--bb-border);border-radius:20px;background:var(--bb-bg)}.promise>span{font-size:2rem}.promise h2{margin:.6rem 0}.promise p,.section-heading>p{color:var(--bb-muted);line-height:1.6}.discovery-section,.guided-search,.catalogue,.how-it-works{margin-top:4rem}.section-heading h2,.catalogue-heading h2{font-size:clamp(2rem,4vw,3.1rem);margin:.3rem 0}.idea-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem;margin-top:1.5rem}.idea-card{display:grid;gap:.5rem;text-align:left;min-height:165px;padding:1.3rem;border:1px solid var(--bb-border);border-radius:20px;background:var(--bb-surface);color:var(--bb-text);cursor:pointer}.idea-card>span{font-size:1.6rem}.idea-card strong{font-size:1.15rem}.idea-card small{color:var(--bb-muted);font-size:.92rem;line-height:1.45}.idea-card.active,.chips button.active{outline:3px solid color-mix(in srgb,var(--bb-primary-dark) 28%,transparent);border-color:var(--bb-primary-dark)}.guided-search{display:grid;grid-template-columns:1fr 1fr;gap:2rem;padding:1.5rem;border:1px solid var(--bb-border);border-radius:24px;background:var(--bb-surface)}.guided-search h2{margin:.25rem 0 1rem}.chips{display:flex;flex-wrap:wrap;gap:.55rem}.chips button,.clear{padding:.6rem .8rem;border:1px solid var(--bb-border);border-radius:999px;background:var(--bb-bg);color:var(--bb-text);cursor:pointer}.catalogue-heading{display:flex;justify-content:space-between;align-items:end;gap:1rem}.filters{display:grid;grid-template-columns:2fr 1fr 1fr auto;gap:.7rem;margin:1rem 0 1.5rem}.filters input,.filters select{min-width:0;padding:.75rem;border:1px solid var(--bb-border);border-radius:12px;background:var(--bb-surface);color:var(--bb-text)}.state{padding:2rem;border:1px solid var(--bb-border);border-radius:18px;background:var(--bb-surface);text-align:center}.error{color:#b33}.product-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:1.2rem}.product-card{overflow:hidden;border:1px solid var(--bb-border);border-radius:20px;background:var(--bb-surface);box-shadow:var(--bb-shadow-sm)}.image-wrap{position:relative;aspect-ratio:4/3;background:var(--bb-bg)}.image-wrap img{width:100%;height:100%;object-fit:cover}.badge{position:absolute;left:.8rem;top:.8rem;padding:.45rem .65rem;border-radius:999px;background:var(--bb-surface);color:var(--bb-text);font-weight:800;text-transform:capitalize}.product-copy{display:grid;gap:.65rem;padding:1.1rem}.product-copy h3{font-size:1.25rem;margin:0}.product-copy>p{margin:0;color:var(--bb-muted);line-height:1.5}.maker{font-size:.78rem!important;text-transform:uppercase;letter-spacing:.08em;font-weight:800;color:var(--bb-primary-dark)!important}.curator-note{display:grid;gap:.2rem;padding:.75rem;border-left:3px solid var(--bb-primary-dark);background:var(--bb-bg);font-size:.9rem}.tag-row{display:flex;gap:.35rem;flex-wrap:wrap}.tag-row span{font-size:.75rem;padding:.3rem .5rem;border-radius:999px;background:var(--bb-bg);text-transform:capitalize}.product-footer{display:flex;justify-content:space-between;align-items:center;gap:.7rem;margin-top:.3rem}.product-footer .btn{padding:.6rem .8rem}.load-more{text-align:center;margin-top:1.5rem}.steps{display:grid;grid-template-columns:repeat(3,1fr);gap:1rem}.steps article{padding:1.2rem;border:1px solid var(--bb-border);border-radius:18px;background:var(--bb-surface)}.steps article>span{display:grid;place-items:center;width:28px;height:28px;border-radius:50%;background:var(--bb-primary-dark);color:white;font-weight:900}.steps p{color:var(--bb-muted);line-height:1.55}@media(max-width:850px){.hero,.guided-search{grid-template-columns:1fr}.idea-grid,.product-grid,.steps{grid-template-columns:1fr 1fr}.filters{grid-template-columns:1fr 1fr}}@media(max-width:560px){.idea-grid,.product-grid,.steps,.filters{grid-template-columns:1fr}.catalogue-heading,.product-footer{align-items:flex-start;flex-direction:column}h1{font-size:2.7rem}}
</style>
